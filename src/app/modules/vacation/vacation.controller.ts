...
import VacationRequest from "./vacation.model";
import { AuthRequest } from "../../middleware/auth";
import Employee from "../employees/employee.model";
import PDFDocument from "pdfkit";

// Employee submits a vacation request
export const requestVacation: RequestHandler = async (
  req: AuthRequest,
  res,
  next,
) => {
  try {
    ...
    if (existingRequest) {
      res.status(400).json({
        message:
          "You already have a vacation request that is Pending or Reviewed. Please wait until it is approved or rejected before submitting a new request.",
      });
      return;
    }
    ...
    let initialStatus: "Pending" | "Reviewed" = "Pending";
    if (
      req.user.role === "Chief Instructor" ||
...
export const getAllVacationRequests: RequestHandler = async (
  req: AuthRequest,
  res,
  next,
) => {
...
export const getMyVacationRequests: RequestHandler = async (
  req: AuthRequest,
  res,
  next
) => {
  try {
    ...
    const requests = await VacationRequest.find({ employee: req.user.id })
...
export const updateVacationStatus: RequestHandler = async (req: AuthRequest, res, next) => {
  try {
    if (!req.user) {
      res.status(401).json({ message: "Unauthorized" });
      return;
    ...
    const request : any = await VacationRequest.findById(requestId);
    if (!request) {
    ...
    // Principal
      ...
      if (status === "Accepted") {
        const vacationEmployee = await Employee.findById(request.employee);
        if (vacationEmployee) {
    ...
    await request.save(); // persists startDate, endDate, numberOfDays, and status
...
export const downloadVacationPDF: RequestHandler = async (req, res, next) => {
  try {
    const { id } = req.params;
    const userId = (req as any).user?.id;

    const vacation : any = await VacationRequest.findById(id).populate("employee");
    ...
    if (vacation.employee._id.toString() !== userId && (req as any).user.role !== "Admin") {
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename="VacationRequest-${vacation._id}.pdf"`
    );
